...
PACKAGE_NAME="lw.PPOCR.Inference-v${VERSION}-linux-x64-opencv"
STAGING_ROOT="${OUTPUT_DIR}/staging-linux-x64"
...
# Linux OpenCV DNN consumes ONNX files only. TensorRT engines belong to the
# Windows TensorRT package and needlessly increase the Linux archive size.
find "${PACKAGE_DIR}/models" -type f -name '*.engine' -delete

cp "${PROJECT_ROOT}/docs/linux-opencv.md" "${PACKAGE_DIR}/LINUX-README.md"
...
if ! compgen -G "${PACKAGE_DIR}/runtimes/linux-x64/opencv/libopencv_*.so.500" \
    >/dev/null; then
  echo "Bundled OpenCV 5.0 shared libraries are missing" >&2
  exit 1
fi
