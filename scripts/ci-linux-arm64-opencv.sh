...
OPENCV_ROOT="${OPENCV_ROOT:-${PROJECT_ROOT}/.ci/opencv-${OPENCV_VERSION}-${CI_DISTRO_ID}-arm64}"
BUILD_DIR="${BUILD_DIR:-${PROJECT_ROOT}/build/ci-${CI_DISTRO_ID}-arm64}"
...
case "${MODE}" in
  opencv)
    ...
    SOURCE_DIR="/tmp/lw-ppocr-opencv-source-${CI_DISTRO_ID}"
    OPENCV_BUILD_DIR="/tmp/lw-ppocr-opencv-build-${CI_DISTRO_ID}"
  ...
  project)
    echo "CI distribution id: ${CI_DISTRO_ID}"
    cat /etc/os-release
