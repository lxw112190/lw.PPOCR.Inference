...
PACKAGE_NAME="lw.PPOCR.Inference-v${VERSION}-linux-arm64-opencv"
STAGING_ROOT="${OUTPUT_DIR}/staging-linux-arm64"
...
# OpenCV DNN consumes ONNX models only. TensorRT engine files are
...
if ! compgen -G \
    "${PACKAGE_DIR}/runtimes/linux-arm64/opencv/libopencv_*.so.500" \
    >/dev/null; then
...
for binary in \
  ...
  if ! readelf -h "${binary}" | grep -q 'Machine:.*AArch64'; then
    echo "Packaged ELF is not AArch64: ${binary}" >&2
    readelf -h "${binary}" >&2
    exit 1
  fi
