...
Linux OpenVINO 配置使用 `backend: "openvino"` 与 `backend_options: {"device":"CPU"}`。当前正式包固定使用 OpenVINO 2025.2.0 CPU，包含私有运行库，不需要目标机器安装 OpenVINO SDK；详见 [Linux OpenVINO 部署说明](linux-openvino.md)。
