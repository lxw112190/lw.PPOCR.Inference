...
int BucketWidth(int required, int minimum) {
    ...
    for (const int bucket : buckets) {
        if (bucket >= minimum && bucket >= required) {
            return bucket;
        }
    }
    return std::max(minimum, required);
}

class Detector {
public:
    Detector(OpenVinoEnvironment& env, const core::ModelStage& model,
    ...
    std::vector<core::OcrRegion> Run(
        ...
        OpenVinoOutput output = session_.Run(input_.data(), input_.size(), shape);
...
class Classifier {
public:
    Classifier(OpenVinoEnvironment& env, const core::ModelStage& model,
        const lw_ppocr_config& config)
        : session_(env, model.path, config.device_id),
...
class Recognizer {
public:
    Recognizer(OpenVinoEnvironment& env, const core::ModelStage& model,
        ...
        for (int index = 0; index < concurrency; ++index) {
            sessions_.push_back(std::make_unique<OpenVinoSession>(
...
private:
    void RunBatch(OpenVinoSession& session, const std::vector<size_t>& indices,
        ...
        const int width = BucketWidth(required_width, minimum_width_);
        std::vector<cv::Mat> resized;
        for (const size_t index : indices) {
        ...
        const auto inference_start = Clock::now();
        OpenVinoOutput output = session.Run(input.data(), input.size(), shape);
        const auto inference_end = Clock::now();
    ...
    std::vector<std::unique_ptr<OpenVinoSession>> sessions_;
...
OpenVinoOcrEngine::OpenVinoOcrEngine(const lw_ppocr_config& config)
    : impl_(std::make_unique<Impl>(config)) {
    Log(LW_PPOCR_LOG_INFO, "OpenVINO Runtime loaded model package: " +
        impl_->manifest.name + ", device: " + impl_->environment.Device());
}

OpenVinoOcrEngine::~OpenVinoOcrEngine() = default;

core::PipelineResult OpenVinoOcrEngine::Run(const lw_ppocr_image& image) {
    ...
    message << "OpenVINO timing: det=" << result.detector.total_ms
...
core::RecognitionResult OpenVinoOcrEngine::RecognizeBatch(
