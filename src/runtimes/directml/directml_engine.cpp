...
public:
    ...
    std::vector<core::OcrRegion> Run(
        ...
        OrtOutput output = session_.Run(input_.data(), input_.size(), shape);
...
private:
    void RunBatch(OrtSession& session, const std::vector<size_t>& indices,
        ...
        const auto inference_start = Clock::now();
        OrtOutput output = session.Run(input.data(), input.size(), shape);
        const auto inference_end = Clock::now();
...
core::PipelineResult DirectMlOcrEngine::Run(const lw_ppocr_image& image) {
...
core::RecognitionResult DirectMlOcrEngine::RecognizeBatch(
