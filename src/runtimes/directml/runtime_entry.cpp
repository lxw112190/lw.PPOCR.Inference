...
using lw::ppocr::directml::DirectMlOcrEngine;
...
lw_ppocr_status Guard(Function&& function, const char* operation) {
    try {
        ...
        g_last_error = std::string(operation) + ": ONNX Runtime: " + exception.what();
    ...
    *result = nullptr;
    return Guard([&] {
        auto owned = lw::ppocr::runtime::ToNativeRecognitionResult(
            static_cast<DirectMlOcrEngine*>(handle)->RecognizeBatch(
                images, image_count));
        *result = &owned->value;
        owned.release();
...
void LW_PPOCR_CALL Destroy(void** handle) {
    ...
    delete static_cast<DirectMlOcrEngine*>(*handle);
    *handle = nullptr;
    g_last_error.clear();
}
