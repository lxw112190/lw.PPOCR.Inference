...
using lw::ppocr::tensorrt::TensorRtOcrEngine;
...
lw_ppocr_status LW_PPOCR_CALL Create(
    ...
    if (!config || !runtime_handle) {
        g_last_error = "TensorRT create received invalid arguments";
        return LW_PPOCR_STATUS_INVALID_ARGUMENT;
    }
    *runtime_handle = nullptr;
    if (config->backend != LW_PPOCR_BACKEND_TENSORRT) {
        g_last_error = "TensorRT Runtime received a different backend id";
        return LW_PPOCR_STATUS_UNSUPPORTED;
    }
    return Guard([&] {
        auto engine = std::make_unique<TensorRtOcrEngine>(*config);
...
lw_ppocr_status LW_PPOCR_CALL Run(
    ...
    if (!handle || !image || !result) {
        g_last_error = "TensorRT run received invalid arguments";
        return LW_PPOCR_STATUS_INVALID_ARGUMENT;
    }
    *result = nullptr;
    return Guard([&] {
        auto owned = NativeResult(
            static_cast<TensorRtOcrEngine*>(handle)->Run(*image));
        *result = &owned->value;
        owned.release();
...
lw_ppocr_status LW_PPOCR_CALL RunJson(void* handle,
    ...
    if (!handle || !image || !json_utf8 || !json_length) {
        g_last_error = "TensorRT JSON run received invalid arguments";
        return LW_PPOCR_STATUS_INVALID_ARGUMENT;
    }
    *json_utf8 = nullptr;
    *json_length = 0;
...
lw_ppocr_status LW_PPOCR_CALL RecognizeBatch(
    ...
    if (!handle || !images || image_count == 0 || !result) {
        g_last_error = "TensorRT recognition received invalid arguments";
        return LW_PPOCR_STATUS_INVALID_ARGUMENT;
    }
    *result = nullptr;
    return Guard([&] {
        auto owned = lw::ppocr::runtime::ToNativeRecognitionResult(
            static_cast<TensorRtOcrEngine*>(handle)->RecognizeBatch(
                images, image_count));
        *result = &owned->value;
        owned.release();
    }, "run TensorRT recognition-only inference");
}
...
lw_ppocr_status LW_PPOCR_CALL GetCapabilities(
    ...
    if (!handle || !capabilities ||
        ...
        g_last_error = "TensorRT capabilities received invalid arguments";
        return LW_PPOCR_STATUS_INVALID_ARGUMENT;
    }
    capabilities->api_version = LW_PPOCR_API_VERSION;
    capabilities->backend = LW_PPOCR_BACKEND_TENSORRT;
    capabilities->backend_name_utf8 = "TensorRT";
    capabilities->supports_cpu = 0;
...
void LW_PPOCR_CALL Destroy(void** handle) {
    ...
    delete static_cast<TensorRtOcrEngine*>(*handle);
    *handle = nullptr;
    g_last_error.clear();
}
