...
`lw.PPOCR.Inference` 是一个统一 PP-OCR 推理项目。项目通过稳定的 C ABI 对外提供一致接口，并将 OpenCV DNN、ONNX Runtime DirectML、便携 ONNX Runtime、OpenVINO、TensorRT 五种推理后端隔离为独立 Runtime。Windows 保持现有四种 Runtime；Linux 已验证 OpenCV DNN 与 ONNX Runtime，并正在增加 OpenVINO CPU Runtime。

当前 Linux 稳定版本为 **v1.2.0 OpenCV DNN**；**v1.3.0 ONNX Runtime CPU/CUDA** 已通过 CI 和 Ubuntu 虚拟机验证，**v1.3.0 OpenVINO CPU** 正在进行 CI 预览。API v1 与 ABI 继续冻结，各 Linux 后端均为独立 Runtime，不改变既有公共接口。部署说明见 [Linux ONNX Runtime CPU/CUDA](docs/linux-onnxruntime.md) 与 [Linux OpenVINO CPU](docs/linux-openvino.md)。

已经发布的 **v1.2.0 Linux OpenCV DNN** 面向 Ubuntu 20.04 x86_64，包含完整 OCR、只识别、HTTP 测试网页和 systemd 服务。构建与虚拟机验证流程见 [Linux OpenCV DNN 部署](docs/linux-opencv.md)。Windows 包不能通过替换几个文件转换成 Linux 包，各 Linux 后端首次部署也应使用对应的完整包。
...
默认只监听 `127.0.0.1:8787`，配置固定为 `"backend":"openvino"` 与 `"backend_options":{"device":"CPU"}`。该预览包暂不开放 OpenVINO GPU。完整使用、API Key、只识别接口、systemd 和排错说明见 [Linux OpenVINO CPU 部署](docs/linux-openvino.md)。
...
当前 Linux 稳定版本为 **v1.2.0 OpenCV DNN**；Linux ONNX Runtime 已通过 CI 与 Ubuntu 虚拟机验证；Linux OpenVINO CPU 正在进行独立 CI 和实机预览验证。`LW_PPOCR_API_VERSION` 保持为 `1`，v1.0.0 ABI 冻结承诺继续有效。
